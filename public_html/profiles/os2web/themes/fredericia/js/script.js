  ...
  $(document).ready(function(){
    ...
    $(window).bind('scroll', function() {
        ...
        if ($(window).scrollTop() > 41 && $(window).width() > 768 ) {
          $('.header_menu header').addClass('navbar-fixed-top');
          $('.header_fixed_inner').addClass('container');
          $('.header_menu header').removeClass('container');
          $('.main-container').css('padding-top','114px');
          $('#fixed-navbar').addClass('row');
          $('img#front-logo').attr('src', Drupal.settings.basePath + Drupal.settings.pathToTheme + '/images/fredericia_logo.png');
       }
        else {
          $('.header_menu header').removeClass('navbar-fixed-top');
          $('.header_fixed_inner').removeClass('container');
          $('.header_menu header').addClass('container');
